...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      else if (i === N - 1) cop = before ? 0 : smooth((pr - 0.24) / 0.36); // let the reveal hold, then bring in the CTA
    ...
    if (near !== activeIndex) {
      ...
      container.dataset.activeIndex = String(near);
    }
  ...
  function raf() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // scrubbing, not a jump cut. For a segment the viewer can't currently see
      // (opacity 0, e.g. scrolled past it, or — the case this was written for —
      // scrolled past the very last section where nothing crossfades in after it)
      // that same slow convergence just means several hundred milliseconds where a
      // stale mid-clip frame sits there instead of the correct settled one, with no
      // motion being hidden by it. Snapping straight to target removes that delay
      // with nothing to lose since it isn't on screen (or wasn't, right up until it
      // reappears already correct).
      if (s.visible) s.cur += (s.target - s.cur) * (reduce ? 1 : 0.18);
      else s.cur = s.target;
